			...
			<el-scrollbar class="scroll-wrap-bar">
                <div class="p-i-section">
                    ...
                    <div class="q-goods-container" v-show="!isAdd">
                        <div class="q-header">
                            <el-input class="info-input" v-model="name" placeholder="请输入报价单名称" size="mini" clearable></el-input>
                            <el-button size="mini" type="primary" @click="createQuotation">生成报价单</el-button>
                            <el-button size="mini" type="warning">插入合同</el-button>
                            <span class="error-info" v-show="!name">* 报价单名称必填</span>
                            <div class="p-s-btn-right" style="float: right">
                                ...
                                <el-button class="info-button" size="mini" type="danger" @click="back">返回报价管理</el-button>
                            </div>
                        </div>
                        <div class="info-warp clearfix">
                            <h3 class="title" :style="{color: showTotalsalesPriceColor}">报价：{{showTotalsalesPrice || 0}} 元</h3>
                            <div class="discount-wrap" v-show="totalsalesPrice > 0">
                                <el-link :underline="false" type="primary">优惠：</el-link>
                                <el-input style="width: 120px" class="discount-input" clearable size="mini" v-model="discount" @input="discountChange" placeholder="折扣: 1 ~ 10"></el-input>
                                <el-input style="width: 120px" class="discount-input" clearable size="mini" v-model="allowance" @input="allowanceChange" placeholder="优惠价"></el-input>
                            </div>
                        </div>
                        <div class="q-goods-list">
                            ...
                            >
                                            ...
                                            <div class="goods-popover">
                                                <table class="organization-table" width="100%" borderColor="#eeeeee" cellspacing='0' cellpadding="5" border="1">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <th width="20%">销售单价(元)</th>
                                                            <td>{{scope.row.salesPrice || '-'}}</td>
                                                        </tr>
                                                         <tr>
                                                            <th width="20%">优惠单价(元)</th>
                                                        ...
                                                        <tr>
                                                            <th width="20%">图片</th>
                                                            <td>
                                                                <viewimg-comp :options="{image: scope.row.goodsImgStr, smallImgShow: true, imgHeight: '60px'}" />
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th width="20%">产品资料</th>
                                                            <td>
                                                                <viewimg-comp :options="{image: scope.row.productImgStr, smallImgShow: true, imgHeight: '60px'}" />
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th width="20%">投标参数</th>
                                                            <td>
                                                                <el-input type='textarea' size="mini" v-model="scope.row.tenderParams" readonly placeholder="-" autocomplete="off"></el-input>
                                                                <!-- {{scope.row.tenderParams || '-'}} -->
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th width="20%">招标参数</th>
                                                            <td>
                                                                <!-- {{scope.row.bidsParams || '-'}} -->
                                                                <el-input type='textarea' size="mini" v-model="scope.row.bidsParams" readonly placeholder="-" autocomplete="off"></el-input>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                ...
                                <el-table-column
                                    ...
                                    label="数量">
                                    <template slot-scope="scope">
                                        <el-input :class="['goods-number', scope.row.number==0 || !/^\d+$/.test(scope.row.number)?'error': null]" size="mini" v-model.number="scope.row.number" placeholder="数量" clearable @input="goodsNumber"></el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column
                                    align="center"
                    ...
                    <div class="all-goods-container" v-show="isAdd">
                        <div class="all-goods-search clearfix">
                            ...
                            <el-button type="primary" size="mini" class="info-button" @click="outerVisible = true">批量搜索</el-button>
                            <div class="p-s-btn-right" style="float: right">
                                <el-button class="info-button" size="mini" type="warning" @click="saveToQuotation">添加到报价单</el-button>
                                <el-button class="info-button" size="mini" type="info" @click="backQuotation">返回报价单</el-button>
                            </div>
                        </div>
                        <el-table 
                            ref="goodslist"
                        ...
                        <more-serach :outerVisible="outerVisible" @success="getName"></more-serach>
                    </div>
                </div>
            </el-scrollbar>
        </div>
...
import lodashUtil from '@/utils/lodash-util'
import MoreSerach from '@/components/upload/more-search'
...
export default {
    ...
    data(){
        return{
            ...
            cacheId: null,
            profitGrade: [], // 利润率等级列表
            profitGradeNumber: null, // 利润率等级数
            showTotalsalesPriceColor: '', // 报价颜色
            colors:['#e7f56c','#F7BA2A','#e91a13'], //评分颜色
        }
    },
    components: { MoreSerach},
    created(){
    ...
    beforeRouteLeave(to,from, next) {
        if(this.qgoodsList.length) {
            this.$confirm('有信息未保存确定要离开吗, 是否继续?', '提示', {
    ...
    watch: {
        'allgoodsSearch': {
            handler(item) {
                this.isHighSearch = validateIsHighSearch(item, 'goodsName')
            },
            deep: true
        }
    },
    methods:{
        ...
        async createQuotation() {
            ...
            const numberError = this.qgoodsList.filter(item => {
            ...
            if(this.profitGradeNumber === 6) {
                this.$confirm('报价利润过低，确定要生成报价单吗, 是否继续?', '风险预警', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
        ...
        save(){
            const _this = this;
            const quotatGoodslist = this.qgoodsList.map(item => ({goodsId: item.goodsId, number: item.number}))
            const params = {
                name: this.name, 
                totalPrice: this.showTotalsalesPrice, 
                profitMargin: this.profitGradeNumber, 
                discount:_this.discount?_this.discount:10,
                allowance:this.allowance, 
                goodsString: JSON.stringify(quotatGoodslist)
            }
            quotationApi.save(params).then(res => {
                this.$notify({
                    title: "成功",
                    message: "报检单生成成功",
                    type: "success",
                    ...
                    onClose() {
                        _this.qgoodsList = []
                        _this.$router.push({name: 'quotation-page'})
                    }
                });
            }).catch(err => console.warn(err))
        },
        ...
        getName(data){
              if(data === false){
                ...
                if(data.radio === "1"){
                    this.allgoodsSearch.goodsName = data.content
                    this.allgoodsSearch.type=""
                }else{
                    this.allgoodsSearch.goodsName=""
                    this.allgoodsSearch.type = data.content
                }
        ...
        computeTotalPrice(goodsList) {
            ...
            goodsList.forEach(item => {
                if(/^\d+$/.test(item.number)) {
                    this.totalsalesPrice += (item.salesPrice*item.number) // 销售价
                    this.totalcostPrice += (item.costPrice*item.number) // 成本价
        ...
        getCacheGoodsList() {
            goodscacheApi.getList({cacheId: this.cacheId}).then(res  => {
                ...
                if(data) this.qgoodsList = data.map(item => ({...item, number: 1}))
                this.qgoodsList.map(item=>{
                    item.orderNum = item.orderNum/2
                    item.dangerNum = item.dangerNum/2
                })
                let _arr = []
                this.qgoodsList.map((item, index) => {
                    _arr.push(Object.assign({},item,{price: item.salesPrice}))
                })
                this.qgoodsList = _arr
        ...
        computeDiscount() {
            ...
            /* 
                ...
                this.discount // 打折
                this.allowance, // 优惠价格
                颜色： 1 绿色 ，2 黄色， 3 橙色， 4红色， 5黑红色 6黑色
                利润率 = (销售价 - 成本价) / 成本价 * 100%
            */
           if(!this.totalcostPrice) {
               this.showTotalsalesPriceColor = ''
               return
           }
           let profit, actualprice; // 利润率 
            if(this.discount && this.allowance) {
                ...
                profit = (actualprice - this.totalcostPrice) / this.totalcostPrice
                this.qgoodsList.forEach((item, index) => {
                    const price = (item.price * parseFloat(this.discount) / 10).toFixed(2)
                    this.$set(this.qgoodsList, index, {...item, price: price})
                })
            ...
            this.showTotalsalesPrice = actualprice.toFixed(2)
            if(profit && profit > 0) {
                const profitGrade = this.profitGrade.filter(item => (profit > parseFloat(item.value))).map(item => item.orderNum)
                this.profitGradeNumber = profitGrade[0]
                switch(this.profitGradeNumber) {
                    case 1:
                        flag = true
                        this.showTotalsalesPriceColor = '#05c70c'
                        break;
                    case 2:
                        flag = true
                        this.showTotalsalesPriceColor = '#ffa500'
                        break;
                    case 3:
                        flag = true
                        this.showTotalsalesPriceColor = '#ff8d00'
                        break;
                    case 4:
                        flag = false
                        this.showTotalsalesPriceColor = '#ff2f00'
                        break;
                    case 5:
                        flag = false
                        this.showTotalsalesPriceColor = '#8c1e05'
                        break;
                    case 6:
                        flag = false
                        this.showTotalsalesPriceColor = '#333333'
                        break;
                }
            } else if(profit <= 0) {
                flag = false
                this.showTotalsalesPriceColor = '#333333'
                this.profitGradeNumber = 6
            } else {
                flag = false
                this.profitGradeNumber = 6
                this.showTotalsalesPriceColor = ''
        ...
        discountChange(e) {
            ...
            return new Promise(resolve => {
                lodashUtil(() => {
                    if(/^[0-9]+$/.test(e) || /^[0-9]+\.?[0-9]+?$/.test(e) && e) {
                        if(parseFloat(e) >= 1 && parseFloat(e) <= 10) {
                            _this.computeDiscount()
                            resolve(true)
                        } else {
                            _this.$notify({
                                title: "打折数错误",
                                message: '请输入合理的打折数: 1~10',
                                type: "error",
                                duration: 2000,
                            })
                            resolve(false)
        ...
        allowanceChange(e) {
            ...
            return new Promise(resolve => {
                lodashUtil(() => {
                    if(/^[0-9]+$/.test(e) || /^[0-9]+\.?[0-9]+?$/.test(e) && e) {
                        if(parseFloat(e) > this.totalsalesPrice) {
                            this.$notify({
                                title: "优惠价错误",
                                message: '优惠价不能大于销售总价',
                                type: "error",
        ...
        /* 利润率数据字典 */
        queryByTypes() {
            dictdataApi.queryByModel({type: 'profit_margin'}).then(res => {
                const data = res.data.data;
                if(data) this.profitGrade = data
